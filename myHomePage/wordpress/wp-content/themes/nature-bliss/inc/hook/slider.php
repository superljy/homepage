...
// Check slider status.
add_filter( 'nature_bliss_filter_slider_status', 'nature_bliss_check_slider_status' );

// Add slider to the theme.
add_action( 'nature_bliss_action_before_content', 'nature_bliss_add_featured_slider', 5 );

// Slider details.
add_filter( 'nature_bliss_filter_slider_details', 'nature_bliss_get_slider_details' );


if ( ! function_exists( 'nature_bliss_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function nature_bliss_get_slider_details( $input ) {

		$featured_slider_type           = nature_bliss_get_option( 'featured_slider_type' );
		$featured_slider_number         = nature_bliss_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = nature_bliss_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = nature_bliss_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'nature-bliss-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = nature_bliss_the_excerpt( apply_filters( 'nature_bliss_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'nature_bliss_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function nature_bliss_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'nature_bliss_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'nature_bliss_filter_slider_details', $slider_details );
		...
		// Render slider now.
		nature_bliss_render_featured_slider( $slider_details );

	}
endif;


if ( ! function_exists( 'nature_bliss_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function nature_bliss_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}
		$featured_slider_transition_effect   = nature_bliss_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = nature_bliss_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = nature_bliss_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = nature_bliss_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = nature_bliss_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = nature_bliss_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = nature_bliss_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = nature_bliss_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'nature_bliss_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function nature_bliss_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = nature_bliss_get_option( 'featured_slider_status' );
